...
- [x] 1.1 與使用者確認 repo 名稱、Routine 執行時間（建議 Asia/Taipei 08:00）、視覺風格偏好
- [x] 1.2 在使用者已建立的空 repo 內初始化 Vite + React + TypeScript 專案結構
- [x] 1.3 加入 `package.json` 依賴：`react`、`react-dom`、`vite`、`@vitejs/plugin-react`、`gray-matter`、`marked`（或 `react-markdown`）、`typescript`
- [x] 1.4 設定 `vite.config.ts` 的 `base` 為 `/<repo-name>/`（GitHub Pages 子路徑）
- [x] 1.5 建立 `posts/` 目錄與 `posts/.gitkeep`
- [x] 1.6 建立 `posts/2026-05-10.md` sample 檔，作為前端開發與 build script 測試資料

## 2. MD → JSON 建置腳本

- [x] 2.1 撰寫 `scripts/build-posts-json.ts`：以 `gray-matter` 解析 frontmatter，並按 section 標題切出三個來源
- [x] 2.2 為每個 section 解析五個欄位（原文網址、原文、繁中改寫、核心概念、前端工程師實際應用），輸出符合 design.md Decision 7 結構的 `posts.json`
- [x] 2.3 處理「（今日無更新）」與「（今日抓取失敗：...）」標記，對應的 post 物件設標記欄位
- [x] 2.4 在 build 時注入 `buildTime`（Asia/Taipei 格式 `YYYY-MM-DD HH:mm`）
...
- [x] 3.8 處理空狀態 UI：「（今日無更新）」「（今日抓取失敗）」需有清楚但不干擾的顯示
- [x] 3.9 視覺設計實作（依 1.1 確認的風格）；至少需 responsive、桌機與手機可用
- [x] 3.10 本機 `npm run dev` 驗證所有互動
...
- [x] 6.4 把 PAT 設定步驟、Routine 維護方法、回滾流程整理成 `README.md` 對外說明
